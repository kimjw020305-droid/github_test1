...
#with st.sidebar를 이용해 코드를 짤 수도 있음
#with st.sidebar():
#    continent=selectbox()
# 2. 입력 폼 생성 (버벅임 방지용 st.form) 여러개의 입력들을 통합해서 하나의 폼으로 만듦
with st.form("search_form"):
    min_pop = st.number_input("최소 인구수를 입력하세요", value=1000000, step=500000)#백만부터 50만 단위로 올릴 수 있음
    submitted = st.form_submit_button("DB 검색 실행")#폼에서는 다른 버튼 함수는 못 쓰고 st.form_submit_button만 쓸 수 있음
...
if submitted:
    ...
    with conn.cursor() as cursor:
        ...
        """ #%s:placeholder,해킹 예방,sql=f"""...{continent}....{min_pop}"""은 해킹에 취약
        # DB 조회 실행
